...
import {getLabelDayByNumber} from '../../../common-ui/common-ui-utils';
...
export class RestService {
   ...
   */
  getOpenWeather(): WeatherInfo {
    let weather: WeatherInfo;
    this.getInfoByIp().pipe(
      take(1),
    ...
    ).subscribe((weatherInfo: OpenWeatherDto) => {
      return weather = {
  ...
  getWeatherStack(): Observable<WeatherInfo> {
    ...
    ).subscribe((weatherInfo: WeatherStackDto) => {
